...
cElement* countMyElements(char* chemCompound);
...
int main (int argc, char *argv[]) {
    ...
    cElement* myElements = countMyElements(chemCompound);
    if(myElements != NULL) {
...
int elementFound(char* chemCompound, char* elementStr, int i, int strLength) {
    ...
    if(isupper(chemCompound[i])) { // If an upper-case letter is found
        ...
        // Check if there is a double-letter element (example: He instead of H, Cl instead of C)
        if(i+1 < strLength && islower(chemCompound[i+1])) { // If so, the second letter must be lowercase
            addCharToString(elementStr, chemCompound[i+1]); // If lowercase found, assign it to the comparison element
        }
...
float readDigits(char* chemCompound, int chemClength,int elemPos) {
    char digitInString[MAX_STRLEN];
    memset(digitInString, 0, MAX_STRLEN);
...
void printElements(cElement* myElements) {
    ...
    for (int i = 1; i < elemNum; i++) {
        printf("%s\t%f\n", myElements[i].name, myElements[i].amount);
    }
}
...
int isWrongSyntax(char* chemCompound) {
    ...
    for (i = 0; i < strlen(chemCompound); i++) {
        ...
        else if (!isalnum(ch) && ch != '(' && ch != ')' && ch != '.' && !isStrEnd(ch)) {
